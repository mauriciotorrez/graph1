...
export const userRoutes = {
  ...
  icon: 'user',
  children: [
    { to: '/users/new', name: 'new', roles: ["ROLE_ADMIN", "ROLE_SUPER_ADMIN"] },
    { to: '/users', name: 'list', roles: ["ROLE_ADMIN", "ROLE_SUPER_ADMIN"] },
  ]
}
...
export const customerRoutes = {
  ...
  icon: 'users',
  children: [
    { to: '/customers/new', name: 'new', roles: ["ROLE_ADMIN", "ROLE_SUPER_ADMIN"] },
    { to: '/customers', name: 'list', roles: ["ROLE_ADMIN", "ROLE_SUPER_ADMIN"] },
  ]
}
...
export const deviceRoutes = {
  ...
  to: '/devices',
  children: [
    { to: '/devices/new', name: 'new', roles: ["ROLE_ADMIN", "ROLE_SUPER_ADMIN"] },
    { to: '/devices', name: 'list', roles: ["ROLE_USER", "ROLE_ADMIN", "ROLE_SUPER_ADMIN"] },
  ]
}
...
export const filterChildren = (userRoles, menu) => {
  const newMenu = menu.map(({ children, ...current }) => {
    if (children === undefined)
      return null;
    const currentChildren = children.filter(child => !child.roles || child.roles.find(role => userRoles.find(userRole => userRole === role)));
    return { ...current, children: currentChildren };
  });
  return newMenu.filter(menuItem => menuItem === undefined || menuItem.children.length > 0);
}
